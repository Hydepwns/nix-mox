#!/usr/bin/env nu
# Unit tests for enhanced-error-handling.nu library
# Tests enhanced error handling compatibility wrapper

use ../../lib/validators.nu *
use ../../lib/logging.nu *

# Test deprecation warning function
export def test_deprecation_warning [] {
    info "Testing deprecation warning function" --context "error-handling-test"
    
    # Test that the deprecation warning function exists and can be called
    try {
        # Import should work even if the underlying implementation is broken
        use ../../lib/enhanced-error-handling.nu __deprecation_warning
        
        # Call the deprecation warning (it just prints, so we test it doesn't crash)
        try {
            __deprecation_warning
            success "deprecation warning function callable" --context "error-handling-test"
            return true
        } catch { |err|
            warn $"Deprecation warning failed: ($err.msg)" --context "error-handling-test"
            return false
        }
    } catch { |err|
        error $"Cannot import deprecation warning function: ($err.msg)" --context "error-handling-test"
        return false
    }
}

export def test_library_import [] {
    info "Testing library import capabilities" --context "error-handling-test"
    
    # Test that we can at least import the library without crashes
    try {
        use ../../lib/enhanced-error-handling.nu
        success "Library import successful" --context "error-handling-test"
        return true
    } catch { |err|
        error $"Library import failed: ($err.msg)" --context "error-handling-test"
        return false
    }
}
...
export def run_enhanced_error_handling_tests [] {
    ...
    let tests = [
        ...
        test_library_import,
        test_compatibility_wrapper_structure,
        test_error_handling_concepts,
        test_try_catch_functionality,
        test_error_propagation,
        test_library_deprecation_status
    ]
