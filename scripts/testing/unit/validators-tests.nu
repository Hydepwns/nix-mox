...
def test_validate_command [] {
    isolated_unit_test_pattern "validate_command" {
...
def test_validate_file [] {
    filesystem_test_pattern "validate_file" {
        # Create test files in isolated environment
        let test_file = ($env.TEST_TMP + "/test_file.txt")
        "test content" | save $test_file
        
        # Test with existing file
        let result = (null | validate_file $test_file)
        assert_equal $result.success true "test file should exist"
        
        # Test with non-existent file
        let result = (null | validate_file ($env.TEST_TMP + "/nonexistent.txt"))
        ...
        let result = (null | validate_file ($env.TEST_TMP + "/optional.txt") --required false)
...
def test_validate_directory [] {
    filesystem_test_pattern "validate_directory" {
        # Create test directories in isolated environment
        let test_dir = ($env.TEST_TMP + "/test_directory")
        mkdir $test_dir
        
        # Test with existing directory
        let result = (null | validate_directory $test_dir)
        assert_equal $result.success true "test directory should exist"
        
        # Test with non-existent directory
        let result = (null | validate_directory ($env.TEST_TMP + "/nonexistent"))
        assert_equal $result.success false "non-existent directory should fail"
        
        # Test with file path (not a directory)
        let test_file = ($env.TEST_TMP + "/test_file.txt")
        "test" | save $test_file
        let result = (null | validate_directory $test_file)
...
def test_validate_env_var [] {
    env_isolation_test_pattern "validate_env_var" {
        ...
        let result = (null | validate_env_var "TEST_ISOLATION_VAR")
    ...
    } {TEST_ISOLATION_VAR: "test_value"} "unit-test"
}
...
def test_compose_validators [] {
    isolated_unit_test_pattern "compose_validators" {
...
def test_input_validation_helpers [] {
    isolated_unit_test_pattern "input_validation_helpers" {
        ...
        let result = (validate_string_input "" "test_param" --allow-empty)
...
def main [] {
    ...
    let test_results = ($test_functions | each { |test_func_name|
        ...
        try {
            let result = (do (get-command $test_func_name).definition | metadata | get name | first | str trim)
    ...
    if $all_passed {
        print $"($env.GREEN)✓ All validators.nu tests passed! (($passed_count)/($total_count))($env.NC)"
        ...
        print $"($env.RED)✗ Some tests failed: ($failed_count)/($total_count)($env.NC)"
        error $"Some tests failed: ($failed_count)/($total_count)" --context "test"
        
        print ""
        print "Failed tests:"
        for test in $failed_tests {
            print $"($env.RED)  - ($test.test_name): ($test.message)($env.NC)"
    ...
    try {
        ...
        if ($temp_dir != null) and ($temp_dir | path exists) {
            # Clean up any remaining temporary files/directories
            ls $temp_dir | where name =~ "nix-mox-.*test.*" | each { |item|
                try {
                    if ($item.type == "dir") {
                        rm -rf $item.name
                    } else {
