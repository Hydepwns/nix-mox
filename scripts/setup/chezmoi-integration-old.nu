...
def create_chezmoi_workflow_scripts [] {
    unified-error-handling info "Creating chezmoi workflow scripts..." "integration"
...
def main [] {
    unified-error-handling info \"Applying chezmoi configuration...\" \"chezmoi-apply\"
    
    let result = (unified-error-handling safe_exec \"chezmoi apply\" \"chezmoi-apply\")
    if \$result.success {
        unified-error-handling success \"Chezmoi configuration applied successfully\" \"chezmoi-apply\"
        unified-error-handling exit_with_success \"Configuration applied\" \"chezmoi-apply\"
    } else {
        unified-error-handling error \"Failed to apply chezmoi configuration\" \"chezmoi-apply\"
...
def main [] {
    unified-error-handling info \"Checking chezmoi differences...\" \"chezmoi-diff\"
    
    let result = (unified-error-handling safe_exec \"chezmoi diff\" \"chezmoi-diff\")
    if \$result.success {
        if (\$result.output | str length) > 0 {
            print \$result.output
            unified-error-handling warning \"Differences found - run 'make chezmoi-apply' to apply them\" \"chezmoi-diff\"
        } else {
            unified-error-handling success \"No differences found - system is up to date\" \"chezmoi-diff\"
        }
    } else {
        unified-error-handling error \"Failed to check chezmoi differences\" \"chezmoi-diff\"
...
def main [] {
    unified-error-handling info \"Syncing chezmoi with remote repository...\" \"chezmoi-sync\"
    
    let result = (unified-error-handling safe_exec \"chezmoi update\" \"chezmoi-sync\")
    if \$result.success {
        unified-error-handling success \"Chezmoi synced successfully\" \"chezmoi-sync\"
        unified-error-handling info \"Run 'make chezmoi-apply' to apply any new changes\" \"chezmoi-sync\"
        unified-error-handling exit_with_success \"Sync completed\" \"chezmoi-sync\"
    } else {
        unified-error-handling error \"Failed to sync chezmoi\" \"chezmoi-sync\"
...
"
    ...
    try {
        ...
        unified-error-handling success "Chezmoi workflow scripts created" "integration"
        true
    } catch { |err|
        unified-error-handling error $"Failed to create workflow scripts: ($err.msg)" "integration"
        false
    }
}

def update_makefile [] {
    unified-error-handling info "Updating Makefile with chezmoi targets..." "integration"
...
'
    ...
    try {
        ...
        unified-error-handling success "Makefile updated with chezmoi targets" "integration"
        true
    } catch { |err|
        unified-error-handling error $"Failed to update Makefile: ($err.msg)" "integration"
        false
    }
}

def create_integration_report [] {
    unified-error-handling info "Creating integration report..." "integration"
...
"
    ...
    try {
        ...
        unified-error-handling success "Integration report created: CHEZMOI_INTEGRATION_REPORT.md" "integration"
        true
    } catch { |err|
        unified-error-handling error $"Failed to create report: ($err.msg)" "integration"
        false
    }
}
...
def main [] {
    ...
    unified-error-handling info "Step 1: Creating chezmoi aliases" "integration"
    let step1_success = (create_chezmoi_aliases)
    
    unified-error-handling info "Step 2: Creating workflow scripts" "integration"
    let step2_success = (create_chezmoi_workflow_scripts)
    
    unified-error-handling info "Step 3: Updating Makefile" "integration"
    let step3_success = (update_makefile)
    
    unified-error-handling info "Step 4: Creating integration report" "integration"
    ...
    if $success_count == $total_steps {
        unified-error-handling success "Integration completed successfully!" "integration"
        unified-error-handling info "Review CHEZMOI_INTEGRATION_REPORT.md for available commands" "integration"
        unified-error-handling info "Try: make chezmoi-diff" "integration"
        unified-error-handling exit_with_success "Integration completed" "integration"
    } else {
        unified-error-handling warning "Integration completed with some failures" "integration"
        unified-error-handling info "Review the output above and CHEZMOI_INTEGRATION_REPORT.md" "integration"
