...
export def aggregate_coverage [] {
    ...
    if ($result_files | is-empty) {
        print "Warning: No test result files found"
        return {
            total_tests: 0
            passed_tests: 0
            failed_tests: 0
...
export def generate_coverage_report [] {
    ...
    let perf_metrics = $coverage_data.performance_metrics
...
export def export_coverage_report [format: string] {
    ...
    let report = {
        ...
        performance: $coverage_data.performance_metrics
...
def generate_recommendations [coverage_data: record] {
    ...
    let avg_duration = $coverage_data.performance_metrics.avg_test_duration
    if $avg_duration > 1.0 {
        $recommendations = ($recommendations | append "Consider optimizing slow tests (>1s average)")
    }

    let slow_tests = ($coverage_data.performance_metrics.test_distribution.slow)
    if $slow_tests > 10 {
...
def generate_html_report [report: record] {
    ...
    <body>
        ...
        <div class='metric'>Coverage Rate: ($report.summary.coverage_rate | into int)%</div>
