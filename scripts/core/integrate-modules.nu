...
def main [] {
    ...
    for ($i | enumerate) in ($modules | enumerate) {
        print $"  {$i.index + 1}. {$i.item.name} - {$i.item.description}"
    }
...
def add-modules-to-templates [] {
    ...
    print "\nAvailable templates:"
    for ($i | enumerate) in ($templates | enumerate) {
        print $"  {$i.index + 1}. {$i.item}"
    }
...
def create-template-with-modules [] {
    ...
    let template_content = $"# {$template_description}
# Template with modules: {$selected_modules | str join ", "}
{{ config, pkgs, ... }}:
{{
  imports = [
    ../profiles/base.nix
