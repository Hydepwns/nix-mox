...
)match $choice "1"=> {}"2"=> {}"3"=> {}"4"=> {}"5"=> {}_=> {print "Invalid choice. Exiting."
exit 1
}}
}
def setup-personal []
...
if (ls config/personal/user.nix
| length )>0{print "⚠️  Personal configuration already exists!"
let response = (input "Do you want to overwrite it? (y/N): "
)
if $response !="y"and$response !="Y"{print "Setup cancelled."
exit 0
}}# Get user input
...
# Environment type (personal, development,
production
)
NIXMOX_ENV=personal
...
GIT_EMAIL=$git_email"$env_config | save .env
...
cp config/templates/gaming.nix
...
cp config/templates/development.nix
config/nixos/configuration.nix
...
cp config/templates/server.nix
config/nixos/configuration.nix
print "\n✅ Server configured!"
print "\n📋 Next steps:"
print "1. Build and switch: sudo nixos-rebuild switch --flake .#nixos"
...
cp config/templates/minimal.nix
config/nixos/configuration.nix
