...
export def is_success [result: record] {
    $result.success == true
}

export def is_error [result: record] {
    $result.success == false
}

export def create_success [message: string, details: record = {}] {
    validation_result true $message $details
}

export def create_error [message: string, context: string, severity: string, code: string, details: record = {}] {
    validation_result false $message $details
}
...
# System resource validator
export def validate_system_resources [
    --min_disk_mb: int = 1024,
    --min_memory_mb: int = 1024,
    --max_cpu_percent: int = 90
] {
    let disk_result = (validate_disk_space (($min_disk_mb / 1024 | into int) + 1))
    let memory_result = (validate_memory (($min_memory_mb / 1024 | into int) + 1))
    
    if ((is_success $disk_result) and (is_success $memory_result)) {
        validation_result true "System resources are sufficient" {
            disk: $disk_result,
...
export def validate_system_enhanced [
    ...
    let resource_result = (validate_system_resources --min_disk_mb ($min_disk_gb * 1024) --min_memory_mb ($min_memory_gb * 1024))
